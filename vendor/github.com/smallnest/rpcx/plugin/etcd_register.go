...
// UnregisterAll a service from etcd but this service still exists in this node.
func (p *EtcdRegisterPlugin) UnregisterAll() (err error) {
	//nodePath := fmt.Sprintf("%s", p.ServiceAddress)
	for _,name := range p.Services{
		path := fmt.Sprintf("%s/%s/%s",p.BasePath,name,p.ServiceAddress)
		_, err = p.KeysAPI.Delete(context.TODO(), path, &client.DeleteOptions{Recursive: true,Dir:true})
		if err != nil {
